...
myMin :: Ord a => [a] -> a
myMin (x:xs) = if length xs == 0 then x else ( if x < myMin xs then x else myMin xs )
...
tuple2List :: [(a, b)] -> [a]
tuple2List (x:xs) = if length xs == 0 then [fst x] else (fst x):(tuple2List xs)
...
myMap :: (a -> b) -> [a] -> [b]
myMap f (x:xs) = if length xs == 0 then [f x] else (f x):(myMap f xs)
...
myFilter :: (a -> Bool) -> [a] -> [a]
myFilter p (x:xs) = if length xs == 0 then p' p x else  (p' p x) ++ (myFilter p xs)
...
myReduce f base (x:xs) = if length xs == 0 then (f x base) else f x (myReduce f base xs)
...
myMapWithReduce f (x:xs) = if length xs == 0 then [myReduce (\x y -> x) (f x) [f x]] else (f x):(myMapWithReduce f xs)