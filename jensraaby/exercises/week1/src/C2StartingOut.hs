...
getThirdElement (_:_:z:_) = z  
-- OK, without cheating:
getThirdElement2 xs = head (tail (tail xs))
...
-- not sure whether this counts as a string is a list of chars...
...
cut low high xs = take (high-low + 1) (drop low xs)
...
fizzBuzz = [if x `mod` 3 == 0 then "Fizz" else if x `mod` 5 == 0 then "Buzz" else show x | x <- [0..100]]
-- I think the above isn't the normal definition of Fizz buzz, but it passes
-- the test :)
